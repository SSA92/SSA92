...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if sensing_info.speed >= 100 and prepare_corner(reli_routeInform):
            ...
            I = abs(map_value(y, 0, 20, 1, 1.5))
            target_pos = pathx[0]
            target_pos = map_value(pathx[0],self.Left_line[0][0],self.Right_line[0][0],-half_load_width,half_load_width)
            print("목표",target_pos)
            ...
            set_steering += 1 * min(1, sensing_info.speed/100) * (target_angle) / (steer_factor + 0.001)
        ...
        path_recommend = local_path_planning(sensing_info.speed, sensing_info.moving_angle, sensing_info.to_middle, mapped_data, half_load_width, sensing_info.track_forward_obstacles)
...
## 2.2 장애물을 바탕으로 경로 분석
def local_path_planning(car_speed, car_yaw, car_pos, forward_map, half_road_width, obstacles) -> int:
    global grid_size
    # grid_size = 0.1 # m
    ...
    if 0<= car_position < num_cells:
        ...
        for idx, point in enumerate(forward_map):
            ...
            for i in range(1,int(1/grid_size *1.6)):  # 차량의 폭을 고려, 그리드가 0.1m 이므로 1m씩 추가 및 여유 0.7m 추가
