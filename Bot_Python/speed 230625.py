...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        target = max(int(sensing_info.speed / 40), 3)
        ...
        ref_angle = math.degrees(math.atan(X_list[target] / Y_list[target]))
        ...
        # pure pursuit 코너에서만 써볼까
        # L = 4.6 # (m)
        # alpha = math.radians(math.degrees(math.atan(X_list[target] / Y_list[target])) - sensing_info.moving_angle)
        # set_steering = math.atan2(2 * L * math.sin(alpha), distance_to_way_points[target]) * 1.5
        ...
        if full_throttle == False:
            ...
            set_throttle = map_value(sensing_info.speed - target_speed, 0, 200, 1, 0)

            if sensing_info.speed - target_speed >= 30:
...
def calculate_obstacles(to_middle, track_forward_angles, distance_to_way_points, track_forward_obstacles):
    """
    경주 게임의 특정 상황에서 장애물의 좌표를 계산하는 함수
    ...
    points = [to_middle, ] + distance_to_way_points
    angles = [0,] + [angle for angle in track_forward_angles]
    ...
    for obj in track_forward_obstacles:
        ...
        if d > 0:
            n, k = int(d // 10), d % 10
            ang = (90 - angles[n] * plag) * math.pi / 180
            obs.append([points[n] * math.sin(sum(ts[:n]) * math.pi / 180) + k * math.sin(ang) - m * math.cos(ang),
                        - points[n] * plag * math.cos(sum(ts[:n]) * math.pi / 180) + k * math.cos(ang) + m * math.sin(ang)])
...
def generate_path(car_speed, car_pos, half_road_width, recommended_path, obstacles):
    global grid_size
    if recommended_path == -1 : return 0
